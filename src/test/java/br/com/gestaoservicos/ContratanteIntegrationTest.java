...
class ContratanteIntegrationTest {
    ...
    @Test void bloqueiaAcessoCruzadoEAceitaMesmoDocumentoEmOutraUnidade() throws Exception {
        ...
        mvc.perform(post("/api/v1/contratantes").header("Authorization", "Bearer " + tokenBob).header("X-Unidade-Id", unidadeBob).contentType(MediaType.APPLICATION_JSON).content(json(contratante("12345678902", "Cliente inválido", false)))).andExpect(status().isBadRequest()).andExpect(jsonPath("$.code").value("VALIDACAO_INVALIDA"));
    }
