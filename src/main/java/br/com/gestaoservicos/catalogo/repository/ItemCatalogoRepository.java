...
public interface ItemCatalogoRepository extends JpaRepository<ItemCatalogo, UUID> {
    @Query("select i from ItemCatalogo i where i.unidade.id = :unidadeId and (:busca is null or lower(i.nome) like lower(concat('%', :busca, '%')) or lower(coalesce(i.codigoReferencia, '')) like lower(concat('%', :busca, '%')))")
