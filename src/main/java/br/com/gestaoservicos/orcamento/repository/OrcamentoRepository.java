...
public interface OrcamentoRepository extends JpaRepository<Orcamento,UUID> {
 ...
 @Query("select o from Orcamento o where o.unidade.id=:unidadeId and (:busca is null or lower(o.numero) like lower(concat('%',:busca,'%'))) and (:status is null or o.status=:status)") Page<Orcamento> buscar(UUID unidadeId,String busca,StatusOrcamento status,Pageable pageable);
}
